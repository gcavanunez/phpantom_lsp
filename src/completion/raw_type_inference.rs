...
/// [`Backend::resolve_variable_assignment_raw_type`], which re-parses the
/// file and returns a PHPStan-style type string (e.g. `"list<User>"`,
/// `"array{name: string, age: int}"`, `"ClassName"`).
...
impl Backend {
    ...
    /// return type).
    pub(super) fn resolve_array_func_element_type(
        func_name: &str,
        args: &ArgumentList<'_>,
        ctx: &VarResolutionCtx<'_>,
    ) -> Option<String> {
    ...
    pub(super) fn extract_argument_text(
    ...
    pub(super) fn try_infer_from_generator_yield(
        return_type: &str,
        ctx: &VarResolutionCtx<'_>,
    ) -> Vec<ClassInfo> {

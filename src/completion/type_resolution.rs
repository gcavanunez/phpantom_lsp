...
///
/// - [`Backend::type_hint_to_classes`]: the public facade that maps a
...
/// - [`Backend::resolve_type_alias`]: fully expands a type alias defined
///   via `@phpstan-type` / `@psalm-type` / `@phpstan-import-type`.
/// - [`Backend::resolve_property_types`]: resolves a property's type hint
///   on a class to all candidate `ClassInfo` values.
/// - [`Backend::resolve_imported_type_alias`]: resolves a single imported
...
use super::resolver::find_class_by_name;
